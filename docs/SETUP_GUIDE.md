...
3-4. `App` 오브젝트의 Inspector에서 빈 칸 채우기 (드래그로):
   - SessionController: App Entry←`App`(자기 자신), Steam Lobby←`App`, Network←`App`, Voice←`App`, Room Config←2-1 에셋
   - SteamLobby: Config←2-2 에셋
   - SteamVoiceService: Config←2-3 에셋
...
5-2. `Player`에 컴포넌트 추가: `NetworkObject`, `NetworkParticipant`, `HeadOrientationSync`,
     `SeatedFirstPersonController`, `PingProbe`, `EmoteSync`.
...
- 음성은 비공간(모두 같은 볼륨) — v1에서 공간 음성으로 교체
